...
static void _restore_emmc_selected(emmcPartType_t restoreType)
{
	...
	if (restoreType & PART_GP_ALL)
	{
		...
		LIST_FOREACH_ENTRY(emmc_part_t, part, &gpt, link)
		{
			...
			emmcsn_path_impl(sdPath, "", part->name, &storage);
			_restore_emmc_part(sdPath, &storage, part, false);
		}
		nx_emmc_gpt_free(&gpt);
	}
	...
	if (restoreType & PART_RAW)
	{
		...
		strcpy(rawPart.name, "rawnand.bin");
		{
			...
			emmcsn_path_impl(sdPath, "", rawPart.name, &storage);
			_restore_emmc_part(sdPath, &storage, &rawPart, true);
		}
	}
	...
	gfx_printf("Time taken: %dm %ds.\n", timer / 60, timer % 60);
	sdmmc_storage_end(&storage);
	if ((restoreType & PART_BOOT) || (restoreType & PART_PRODINFO || restoreType & PART_PRODINFO_ONLY)) goto out;
		else {
		gfx_printf("\n%kComplete.%k\n\nRestore BOOT0/1 from folder %d?\n\n%k[POWER] - Yes\n%k[VOL+/-] - No\n", 0xFF00FF00, 0xFFFFFFFF, folder, 0xFF00FF00, 0xFFFFFF00);
		u32 btn = btn_wait();
		if (btn & BTN_POWER){
