...
void tui_pbar(int x, int y, u32 val, u32 fgcol, u32 bgcol)
{
	...
	gfx_printf("%k[%3d%%]%k", fgcol, val, MAIN_TEXT_COL);
...
void *tui_do_menu(menu_t *menu)
{
	...
	while (true)
	{
		...
		if (h_cfg.errors)
		{
			gfx_printf("%k Warning: %k", DANGER_ERROR_TEXT_COL, MAIN_TEXT_COL);
			if (h_cfg.errors & ERR_LIBSYS_LP0)
				gfx_printf("Sleep mode library is missing!\n");
		}
		gfx_con_setpos(0,  1191);
		gfx_printf("%k [VOL] Up / Down\n [PWR] Select Option \n [PWR][VOL+] Screenshot%k", STATIC_TEXT_COL, MAIN_TEXT_COL);
