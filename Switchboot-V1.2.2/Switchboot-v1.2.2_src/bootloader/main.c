...
void launch_tools(u8 type)
{
	...
	if (sd_mount())
	{
		...
		if (type == 0){
			...
			memcpy(dir, "argon/payloads", 15);
		} else if (type == 3 || type == 13){
			memcpy(dir, "atmosphere", 11);
		} else if (type == 4 || type == 14){
			memcpy(dir, "bootloader/ini", 15);
	...
	{
		if (type != 11){
		memcpy(dir + strlen(dir), "/", 2); // remove extra / in SD root entry
		}
		...
		} else if (type >= 10)
		{
			...
			if (res){
				gfx_printf (&gfx_con, "Failed creating config entry.\n\nPress any key.");
				return;
			}
			FIL fp;
			if (!sd_mount())
			...
			if (f_open(&fp, "bootloader/hekate_ipl.ini", FA_WRITE | FA_READ) != FR_OK){
				gfx_printf (&gfx_con, "Failed opening hekate_ipl.ini\n\nPress any key.");
				return;
				}
...
//got to be a better way of doing this. Struct maybe?
void type1() {launch_tools(1); }; void type11() {launch_tools(11); };
void type2() {launch_tools(2); }; void type12() {launch_tools(12); };
void type3() {launch_tools(3); }; void type13() {launch_tools(13); };
void type4() {launch_tools(4); }; void type14() {launch_tools(14); };
void type5() {launch_tools(5); }; void type15() {launch_tools(15); };
void type6() {launch_tools(6); }; void type16() {launch_tools(16); };
void type7() {launch_tools(7); }; void type17() {launch_tools(17); };
void type8() {launch_tools(8); }; void type18() {launch_tools(18); };
void type9() {launch_tools(9); }; void type19() {launch_tools(19); };
...
ment_t ment_options[] = {
	MDEF_BACK(),
	MDEF_CAPTION("-- Autoboot Options --", 0xFF00FF00),
	MDEF_CHGLINE(),
...
ment_t ment_backup_choose_folder[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_CAPTION("BOOT0/1 will be saved to", 0xFF00FF00),
...
menu_t menu_backup_choose_folder = {
	...
	"Choose a folder to work with", 0, 0
};
...
ment_t ment_tools[] = {
	...
	MDEF_MENU("Choose backup / restore folder", &menu_backup_choose_folder),
	...
	//MDEF_HANDLER("Factory Reset / Erase", factory_reset),
	MDEF_END()
};
...
ment_t ment_addini[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_CAPTION("Choose a folder to scan", 0xFF00FF00),
	MDEF_CHGLINE(),
	MDEF_CHGLINE(),
	MDEF_HANDLER("SD:/", type11),
	MDEF_HANDLER("SD:/argon/payloads", type12),
	MDEF_HANDLER("SD:/atmosphere", type13),
	MDEF_HANDLER("SD:/bootloader/ini", type14),
	MDEF_HANDLER("SD:/bootloader/payloads", type15),
	MDEF_HANDLER("SD:/sept", type16),
	MDEF_HANDLER("SD:/switch", type17),
	MDEF_HANDLER("SD:/sxos", type18),
	//MDEF_HANDLER("SD:/", type19),
	MDEF_END()
};
...
ment_t ment_launch[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_CAPTION("Choose a folder to scan or", 0xFF00FF00),
	MDEF_MENU("Add entry to ini", &menu_addini),
	MDEF_CHGLINE(),
	MDEF_HANDLER("SD:/", type1),
	MDEF_HANDLER("SD:/argon/payloads", type2),
	MDEF_HANDLER("SD:/atmosphere", type3),
	MDEF_HANDLER("SD:/bootloader/ini", type4),
	MDEF_HANDLER("SD:/bootloader/payloads", type5),
	MDEF_HANDLER("SD:/sept", type6),
	MDEF_HANDLER("SD:/switch", type7),
	MDEF_HANDLER("SD:/sxos", type8),
	//MDEF_HANDLER("SD:/", type9),
	MDEF_CHGLINE(),
...
ment_t ment_top[] = {
	...
	MDEF_CAPTION("---------------------", 0xFF444444),
	MDEF_MENU("Payload Menu", &menu_launch),
	MDEF_CAPTION("---------------------", 0xFF444444),
	...
	MDEF_CAPTION("---------------------", 0xFF444444),
	MDEF_HANDLER("SAMD Chip update mode", restore_septprimary_dat),
	MDEF_CAPTION("---------------------", 0xFF444444),
	MDEF_HANDLER("About", about),
	MDEF_CAPTION("---------------------", 0xFF444444),
	MDEF_CHGLINE(),
	MDEF_CAPTION("Autorun:  payload.bin", 0xFF00FF00),
	MDEF_CAPTION("         payload1.bin", 0xFF00FF00),
	MDEF_CHGLINE(),
	MDEF_CAPTION("Mode:               ?", 0xFF00FF00),
	MDEF_CHGLINE(),
	MDEF_CAPTION("USB Strap:         NO", 0xFF00FF00),
	MDEF_CHGLINE(),
	MDEF_CAPTION("VOL+ Strap:        NO", 0xFF00FF00),
	MDEF_CHGLINE(),
	MDEF_CAPTION("Joycon Strap:      NO", 0xFF00FF00),
	MDEF_END()
};
menu_t menu_top = {
	ment_top,
	"Switchboot v1.2.2 - Hekate CTCaer v4.9.1", 0, 0
};
