...
if (sd_mount())
	...
	{	
		...
		file_sec = file_browser("bootloader/payloads", NULL, "Choose a payload", false);
	}	
...
void ini_list_launcher()
{
	...
	if (sd_mount())
	{
		path_sec = file_browser("bootloader/ini", NULL, "Choose folder / INI", false);
	...
	if(sd_mount())
	{
		if(!f_stat("sxos/emunand", &fno))
		{
			switch (selection)
			{
				...
				case 2:
				{
					browse_dir = file_browser("sxos/MultiNAND", NULL, "Choose a slot.", true);
					...
					if (fr) {gfx_printf("Files not moved. Empty?\n");}
					if(!fr)
					{
						memcpy(enandx+enandx_len, "active", 7);
...
ment_t ment_cinfo[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_CAPTION("-- System Info --", INFO_TEXT_COL),
...
ment_t ment_restore[] = {
	...
	MDEF_HANDLER("Prodinfo ONLY (safe folder)", restore_emmc_quick_prodinfo),
	MDEF_CHGLINE(),
	MDEF_CAPTION("-- Full --", INFO_TEXT_COL),
	MDEF_HANDLER("BOOT0/1", restore_emmc_boot),
	MDEF_HANDLER("Rawnand(rawnand.bin)", restore_emmc_rawnand),
	MDEF_CHGLINE(),
	MDEF_CAPTION("-- Partitions --", INFO_TEXT_COL),
	MDEF_HANDLER("SYSTEM/USER", restore_emmc_gpp_parts),
	MDEF_CHGLINE(),
...
ment_t ment_backup[] = {
	...
	MDEF_CHGLINE(),
	MDEF_CAPTION("-- Full --", INFO_TEXT_COL),
	MDEF_HANDLER("BOOT0/1", dump_emmc_boot),
	MDEF_HANDLER("Rawnand (rawnand.bin)", dump_emmc_rawnand),
	MDEF_CHGLINE(),
	MDEF_CAPTION("-- Partitions --", INFO_TEXT_COL),
	MDEF_CHGLINE(),
	MDEF_HANDLER("SYSTEM", dump_emmc_system),
	MDEF_HANDLER("USER", dump_emmc_user),
	MDEF_END()
};
...
ment_t ment_sxos[] = {
	...
	MDEF_CAPTION("-- MultiNAND Options --", INFO_TEXT_COL),
	MDEF_CHGLINE(),
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_HANDLER("MultiNAND setup", emunand_1),
	MDEF_HANDLER("Activate slot", emunand_2),
	MDEF_HANDLER("Deactivate emuNAND", emunand_3),
...
ment_t ment_top[] = {
	...
	MDEF_CAPTION("------------", STATIC_TEXT_COL),
	MDEF_MENU("SXOS Options / MultiNAND", &menu_sxos),
	MDEF_CAPTION("------------------------", STATIC_TEXT_COL),
...
menu_t menu_top = { ment_top, "Switchboot v1.4.2 - Hekate CTCaer v5.0.1", 0, 0 };
