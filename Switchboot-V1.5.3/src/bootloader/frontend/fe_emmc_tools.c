...
void reboot_samd21_update(){
	...
	if (!sd_mount()){
		gfx_printf("\nThis requires an SD card.\n\nInsert one now, or reboot to RCM\nand hold [VOL+] for 10 seconds");
		return;
	}
	//does it exist?
	...
	if (f_stat("bootloader/payloads/Samd21_Update.bin", NULL) == FR_OK){
		...
		gfx_printf("\nEnsure USB is unplugged!\n\nPress [RESET] on chip twice or hold [VOL+]\nfor 10 seconds... THEN connect to USB.\n\nHold [PWR] for 12 seconds when finished.\n\n[PWR] - Reboot now");
		u8 btn = btn_wait();
