...
char *file_browser(char* start_dir, char* required_extn, char* browser_caption, bool get_dir_only, bool goto_root_on_fail){
		...
		while(true){
			...
			if (filelist)
			{
				...
				while (true)
				{
					...
					ments[i + 9].caption = &filelist[i * 256];
					ments[i + 9].data = &filelist[i * 256];
			...
			if (i > 0)
			{
				...
				if (!strcmp(file_sec_untrimmed + strlen(file_sec_untrimmed) - 11,"<home_menu>")) {home_selected = true; break;}
...
if (home_selected)goto out2;	
if (goto_root_on_fail){		
	if(!filelist && start_dir != NULL && i == 0) 
	{
		gfx_printf("\nNo valid files in %s.\n\nGoing to SD ROOT", start_dir);
...
if(required_extn)
	...
	if ((!dir_selected) &&(strcmp(browse_dir+strlen(browse_dir) - strlen(required_extn), required_extn)))
			{gfx_printf("\nNo %s file in this location.", required_extn); msleep(2000); browse_dir[0] = '\0';goto start;}	
	}
if(get_dir_only){	
