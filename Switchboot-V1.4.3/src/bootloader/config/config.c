...
static void _config_autoboot_list(void *ent)
{
	...
	if (sd_mount())
	{
		path_sec = file_browser(NULL, NULL, "Choose folder / INI", false, true);
		if (!path_sec) goto out2;
		u8 fr1 = 0; u8 fr2 = 0;
		
		fr1 = (ini_parse(&ini_sections, path_sec, true));
		if(!fr1) {fr2 = (ini_parse(&ini_sections, path_sec, false));}
	...
	if (sd_mount())
	{
		if (ini_parse(&ini_sections, "bootloader/hekate_ipl.ini", false))
		{
			...
			if (h_cfg.autoboot_list)
				ments[3].caption = "*Other configuration selected. Change HERE.";
			else
				ments[3].caption = " Browse SD for Other configs / INI";
		...
		{
			EPRINTF("Could not open selected INI.");
			goto out;
		}
	}
...
void config_bootdelay()
{
	...
	for (i = 1; i < delay_entries; i++)
	{
		...
		memcpy(delay_text + i * 32 + 2, " sec.", 6);
	...
	menu_t menu = {ments, "Delay for entering menu", 0, 0};
...
void config_verification()
{
	...
	memcpy(vr_text + 64,  " Quick (safe)", 14);
	memcpy(vr_text + 128, " Slow (extra safe)", 19);
...
void config_auto_hos_poweroff()
{
	...
	menu_t menu = {ments, "Prevent reboot after power-off", 0, 0};
