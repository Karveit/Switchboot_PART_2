...
key_output: ;
    ...
    gfx_printf("\nFound %d keys. Please wait...\n", _key_count);
		...
		if (savetosd == true)
		{
			...
			text_buffer = (char *)calloc(0x4000, 1);
			buf_index = 0;
			//save choi friendly keys.txt
			SAVE_KEY_FAMILY("master_key", master_key, MAX_KEY, 0x10);
			SAVE_KEY("header_key", header_key, 0x20);
			SAVE_KEY("aes_kek_generation_source", aes_kek_generation_source, 0x10);
			SAVE_KEY("aes_key_generation_source", aes_key_generation_source, 0x10);	
			...
			SAVE_KEY("key_area_key_system_source", fs_keys[4], 0x10);
			SAVE_KEY("package2_key_source", package2_key_source, 0x10);
			gfx_printf("\nCreated ChoiDujour keys.txt\n");
			if (!sd_save_to_file(text_buffer, buf_index, "sd:/keys.txt")){
					gfx_printf("Written keys.txt to SD root.\n");
					free(text_buffer);
					sdmmc_storage_end(&storage);
			}
			
		} else if (savetosd == false){
		gfx_printf("Skipped updating key files\n");
		}	
