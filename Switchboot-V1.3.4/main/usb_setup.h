...
void sendPayload(const byte *payload, uint32_t payloadLength)
{
  ...
  if (hekate == true){
  usbBufferedWrite(HKPART1, 99952);
  usbBufferedWrite(payload, payloadLength);
  usbBufferedWrite(inbetween_payloads, 3068);
  usbBufferedWrite(payload, payloadLength);
  
  usbBufferedWrite(MODE, 19);
  ...
  usbBufferedWrite(VOLPLUSSTRAP,19);
  #ifdef DONGLE
  usbBufferedWrite(NOCHIPFOUND,14);
  #else
 ...
 usbBufferedWrite(JOYCONSTRAP,19);
 #ifdef DONGLE
 usbBufferedWrite(NOCHIPFOUND,14);
 #else
...
#endif
 usbBufferedWrite(TWOBYTES, 2);
 usbBufferedWrite(BOARDNAME, 31);
 usbBufferedWrite(PART3, 22910);
  } else 
  if (argon == true){
    usbBufferedWrite(payload, payloadLength);
  }
