	...
	else
	{
		...
		else
		{
			gfx_printf("%kExtended CSD V1.%d:%k\n",
				INFO_TEXT_COL, storage.ext_csd.ext_struct, MAIN_TEXT_COL);
			...
			LIST_FOREACH_ENTRY(emmc_part_t, part, &gpt, link)
			{
				gfx_printf(" %02d: %k%s%k\n     Size: % 5d MiB (LBA Sectors 0x%07X)\n     LBA Range: %08X-%08X\n",
					gpp_idx++, 0xFFAEFD14, part->name, MAIN_TEXT_COL, (part->lba_end - part->lba_start + 1) >> SECTORS_TO_MIB_COEFF,
...
void print_sdcard_info()
{
	...
	if (sd_mount())
	{
		...
		gfx_printf("%kCard-Specific Data V%d.0:%k\n", INFO_TEXT_COL, sd_storage.csd.structure + 1, MAIN_TEXT_COL);
		...
		gfx_printf("%kFound %s volume:%k\n Free:    %d MiB\n Cluster: %d KiB\n",
				INFO_TEXT_COL, sd_fs.fs_type == FS_EXFAT ? "exFAT" : "FAT32", MAIN_TEXT_COL,
