...
void send_switchboot(const byte *payload)
{
  ...
  usbBufferedWrite(SW1, 102233);
  usbBufferedWrite(payload, 12);
  usbBufferedWrite(SW2, 459);
  bool por_active = false;
  if(EEPROM_USB_POR == 1 && currentTime < (LOOK_FOR_TEGRA_SECONDS * 1000))//assume RCM if under searchtime
  {
    usbBufferedWrite(usb_por_enabled, 37);
    por_active = true;
  }
  if(EEPROM_USB_POR == 0)//assume RCM if under searchtime
  {
    usbBufferedWrite(usb_por_disabled, 37);
  }

  usbBufferedWrite(SW3, 491);

  #ifdef TXTFILEPROVIDESTRAPVALUES
  usbBufferedWrite(STRAPINFOTXTNEEDED,32);
  #else
  usbBufferedWrite(STRAPINFOTXTNOTNEEDED,32);
  #endif

  usbBufferedWrite(SW4, 540);

  usbBufferedWrite(payload, 12);

  usbBufferedWrite(ZEROES, 4);
  
  if (EEPROM_PAYLOAD_NUMBER == 1) usbBufferedWrite(DISABLED1,13);
  if (EEPROM_PAYLOAD_NUMBER == 2) usbBufferedWrite(DISABLED2,13);
  if (EEPROM_PAYLOAD_NUMBER == 3) usbBufferedWrite(DISABLED3,13);
  if (EEPROM_PAYLOAD_NUMBER == 4) usbBufferedWrite(DISABLED4,13);
  if (EEPROM_PAYLOAD_NUMBER == 5) usbBufferedWrite(DISABLED5,13);
  if (EEPROM_PAYLOAD_NUMBER == 6) usbBufferedWrite(DISABLED6,13);
  if (EEPROM_PAYLOAD_NUMBER == 7) usbBufferedWrite(DISABLED7,13);
  if (EEPROM_PAYLOAD_NUMBER == 8) usbBufferedWrite(DISABLED8,13);

  usbBufferedWrite(CURRENT, 11);
  
  usbBufferedWrite(payload, 12);

  usbBufferedWrite(RESTORED_CURRENT, 20);
  
  usbBufferedWrite(payload, 12);

  usbBufferedWrite(BEFORE_STRAPS, 1152);
