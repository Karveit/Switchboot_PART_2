  ...
  while (VOL_TICK_TIMER < MASTER_VOLUP_TIMER) {
    ...
    if (volup_pressed) {
      ...
      if (VOL_TICK_TIMER < LONG_PRESS_TRIGGER1) {
        ...
        setLedColor("green");
      } else if (VOL_TICK_TIMER > LONG_PRESS_TRIGGER3 && VOL_TICK_TIMER < LONG_PRESS_TRIGGER4) {
        setLedColor("white");
      } else if (VOL_TICK_TIMER > LONG_PRESS_TRIGGER4 && VOL_TICK_TIMER < LONG_PRESS_TRIGGER5) {
        setLedColor("red");
      }
...
void disable_chip() {
  bool res = EEPROM_CHIP_DISABLED.read();
  if (res){
  EEPROM_CHIP_DISABLED.write(false);
  } else {
