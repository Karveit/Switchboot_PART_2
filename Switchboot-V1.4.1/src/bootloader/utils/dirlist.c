...
char *dirlist(const char *directory, const char *pattern, bool includeHiddenFiles, bool includeDirs)
{
	u8 max_entries = 61;
	...
	if (!pattern && !f_opendir(&dir, directory))
	{
		for (;;)
		{
			...
			if (res || !fno.fname[0])
				break;
			if ((!(fno.fattrib & AM_DIR) && (fno.fname[0] != '.') && (includeHiddenFiles || !(fno.fattrib & AM_HID))) && !(includeDirs))
			{
