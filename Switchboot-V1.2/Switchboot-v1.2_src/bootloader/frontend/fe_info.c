	...
	else
	{
		...
		else
		{
			gfx_printf(&gfx_con, "%kExtended CSD V1.%d:%k\n",
				0xFF00FF00, storage.ext_csd.ext_struct, 0xFFFFFFFF);
...
void print_sdcard_info()
{
	...
	if (sd_mount())
	{
		...
		gfx_printf(&gfx_con, "%kCard-Specific Data V%d.0:%k\n", 0xFF00FF00, sd_storage.csd.structure + 1, 0xFFFFFFFF);
		...
		gfx_printf(&gfx_con, "%kFound %s volume:%k\n Free:    %d MiB\n Cluster: %d KiB\n",
				0xFF00FF00, sd_fs.fs_type == FS_EXFAT ? "exFAT" : "FAT32", 0xFFFFFFFF,
...
void print_tsec_key()
{
	...
	gfx_printf(&gfx_con, "%kTSEC key:  %k", 0xFF00FF00, 0xFFFFFFFF);
	...
	if (res >= 0)
	{
		...
		if (pkg1_id->kb == KB_FIRMWARE_VERSION_620)
		{
			gfx_printf(&gfx_con, "\n%kTSEC root: %k", 0xFF00FF00, 0xFFFFFFFF);
			for (u32 j = 0; j < 0x10; j++)
