...
void dump_keys() {
    ...
    if (pkg1_id->kb >= KB_FIRMWARE_VERSION_700) {
        ...
        if (!(EMC(EMC_SCRATCH0) & EMC_SEPT_RUN)) {
			gfx_printf(&gfx_con, " Sept needs to run.\n\n Please re-run this option\n\n once rebooted.");
			msleep(2000);
            FIL fp;
...
dismount:
	...
	gfx_printf(&gfx_con, "%kall factory settings?\n\n", 0xFFFFFFFF);
	gfx_printf(&gfx_con, "%k[PWR] Yes ---- [VOL] No\n\n", 0xFFFFFFFF);
	btn_wait();
	u32 i = 0;
	char fileno[100];
	while (i < 5000){
		char *path = "emmc:/save/8000000000000043";	
		itoa(i, fileno, 10);
		memcpy (path + strlen(path - strlen(fileno)), fileno, 4);
	gfx_printf(&gfx_con, path);
	gfx_printf(&gfx_con, "\n");
	msleep(100);
	++i;
	}*/
