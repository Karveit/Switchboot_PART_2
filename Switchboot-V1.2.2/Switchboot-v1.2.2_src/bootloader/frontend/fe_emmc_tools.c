...
static void _dump_emmc_selected(emmcPartType_t dumpType)
{
	...
	if (!(dumpType & PART_PRODINFO || dumpType & PART_PRODINFO_ONLY))
		{
		EPRINTFARGSGREEN("Folder %d is selected.\n", folder);
		EPRINTFARGSGREEN("Backup will be saved to BACKUP_%d\n\n", folder);
		} else {
	...
	emmcsn_path_impl(sdPath, "/BOOTS", "", &storage);
