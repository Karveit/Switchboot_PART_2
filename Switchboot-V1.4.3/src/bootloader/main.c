...
if (sd_mount())
	...
	{	
		...
		file_sec = file_browser("bootloader/payloads", NULL, "Choose a payload", false, true);
		if (!file_sec) return;
	}	
...
void ini_list_launcher()
{
	...
	if (sd_mount())
	{
		path_sec = file_browser("bootloader/ini", NULL, "Choose folder / INI", false, true);
		if (!path_sec) return;
		u8 fr1 = 0; u8 fr2 = 0;
	...
	if(sd_mount())
	{
		if(!f_stat("sxos/emunand", &fno))
		{
			switch (selection)
			{
				...
				case 2:
				{
					browse_dir = file_browser("sxos/MultiNAND", NULL, "Choose a slot.", true, false);
					if (!browse_dir) break;
...
ment_t ment_sxos[] = {
	...
	MDEF_HANDLER("Deactivate slot", emunand_3),
...
menu_t menu_top = { ment_top, "Switchboot v1.4.3 - Hekate CTCaer v5.0.1", 0, 0 };
