	...
	if (dumpType & PART_BOOT)
	{
		...
		for (i = 0; i < 2; i++)
		{
			...
			gfx_printf("%k%02d: %s (%07X-%07X)%k\n", MAIN_TEXT_COL, i,
				bootPart.name, bootPart.lba_start, bootPart.lba_end, MAIN_TEXT_COL);
	...
	if ((dumpType & PART_SYSTEM) || (dumpType & PART_USER) || (dumpType & PART_RAW) || (dumpType & PART_PRODINFO))
	{
		...
		if ((dumpType & PART_SYSTEM) || (dumpType & PART_USER))
		{
			...
			LIST_FOREACH_ENTRY(emmc_part_t, part, &gpt, link)
			{
				...
				gfx_printf("%k%02d: %s (%07X-%07X)%k\n", MAIN_TEXT_COL, i++,
					part->name, part->lba_start, part->lba_end, MAIN_TEXT_COL);
		...
		if (dumpType & PART_RAW)
		{
			...
			{
				gfx_printf("%k%02d: %s (%07X-%07X)%k\n", MAIN_TEXT_COL, i++,
					rawPart.name, rawPart.lba_start, rawPart.lba_end, MAIN_TEXT_COL);
	...
	if (dumpType & PART_PRODINFO)
		{
			...
			{
				gfx_printf("%k%02d: %s (%07X-%07X)%k\n", MAIN_TEXT_COL, i++,
					prodinfoPart.name, prodinfoPart.lba_start, prodinfoPart.lba_end, INFO_TEXT_COL);

				emmcsn_path_impl(sdPath, "/safe", prodinfoPart.name, &storage);
		...
		bootPart.lba_end = (BOOT_PART_SIZE / NX_EMMC_BLOCKSIZE) - 1;
			...
			gfx_printf("%k%02d: %s (%07X-%07X)%k\n", MAIN_TEXT_COL, i,
				bootPart.name, bootPart.lba_start, bootPart.lba_end, INFO_TEXT_COL);

			sdmmc_storage_set_mmc_partition(&storage, i + 1);

			emmcsn_path_impl(sdPath, "/safe", bootPart.name, &storage);
			res = _dump_emmc_part(sdPath, &storage, &bootPart);
			}
		}
	}
...
static void _restore_emmc_selected(emmcPartType_t restoreType)
{
	...
	{
		gfx_con_setpos(gfx_con.savedx, gfx_con.savedy);
		gfx_printf("%kWait... (%ds)    %k", INFO_TEXT_COL, failsafe_wait, MAIN_TEXT_COL);
		msleep(1000);
		failsafe_wait--;
	}
	gfx_con_setpos(gfx_con.savedx, gfx_con.savedy);
...
void reboot_samd21_update(){
	...
	//does it exist?
	if (f_stat("bootloader/payloads/Samd21_Update", NULL) == FR_OK){
		gfx_printf("\nDummy payload exists!\n\n");
	} else {
		gfx_printf("\nPlease wait\n\n");
		sd_save_to_file(dummy_bin, sizeof(dummy_bin), "bootloader/payloads/Samd21_Update");
	...
	if (btn & BTN_POWER) launch_payload("bootloader/payloads/Samd21_Update", false);
	return;
}
