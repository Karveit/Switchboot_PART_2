...
if (sd_mount())
	{
		...
		if (type == 4)
		{
			if (!f_rename("disabled.bin", "payload.bin")){gfx_printf ("Current payload.bin restored."); goto out;}
			else if (!f_rename("payload.bin", "disabled.bin")){gfx_printf ("Current payload.bin disabled"); goto out;}
			else {gfx_printf ("File not found."); goto out;}
		}
		if (type == 5)
		{
			if (!f_rename("disabled1.bin", "payload1.bin")){gfx_printf ("Current payload1.bin restored."); goto out;}
			else if (!f_rename("payload1.bin", "disabled1.bin")){gfx_printf ("Current payload1.bin disabled"); goto out;}
			else {gfx_printf ("File not found."); goto out;}
		}
...
void batt_check()
{
	int battVoltage, battPercent, Current;

	max17050_get_property(MAX17050_VCELL, &battVoltage);
	max17050_get_property(MAX17050_RepSOC, (int *)&battPercent);
	while(((battPercent >> 8) & 0xFF) < 1)
	{
		max17050_get_property(MAX17050_VCELL, &battVoltage);
		max17050_get_property(MAX17050_RepSOC, (int *)&battPercent);
		...
		gfx_con_setpos(32, 1195);
		gfx_printf("%k%c%c", 0xFFFFFFFF, 0x82, 0x81);
		gfx_con_setpos(32, 1180);
		gfx_printf("%k%c%c", 0xFFFFFFFF, 0x82, 0x81);
		gfx_con_setpos(32, 1165);
		gfx_printf("%k%c%c", 0xFFFFFFFF, 0x84, 0x83);
if(Current>0)
		{
		gfx_con_setpos(32, 1210);
		gfx_printf("%k%c%c", 0xFF00FF00, 0x80, 0x7F);
		msleep(500);
		gfx_con_setpos(32, 1210);
		gfx_printf("%k%c%c", 0xFF008800, 0x80, 0x7F);
		msleep(500);
		}
		else
		{
		gfx_con_setpos(32, 1210);
		gfx_printf("%k%c%c", 0xFFFF0000, 0x80, 0x7F);
		msleep(4000);
		power_off();
		}
...
ment_t ment_options[] = {
	...
	MDEF_HANDLER("Disable / Enable payload1.bin", type5),
	...
	MDEF_HANDLER("Prevent reboot after USB unplug", config_usb_poweronreset),
...
ment_t ment_cinfo[] = {
	...
	MDEF_HANDLER("Save TSEC key", print_tsec_key),
	MDEF_CHGLINE(),
...
menu_t menu_top = { ment_top, "Switchboot_UF2 v1.5.3 - Hekate v5.0.2", 0, 0 };
